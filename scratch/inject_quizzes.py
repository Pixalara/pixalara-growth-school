...
# Define the 15 Monitoring & Observability questions
    ...
    html = """
            ...
            <section class="quiz-section" style="margin-top: 50px; padding-top: 40px; border-top: 2px solid #eee;">
                ...
                <p style="color: #666; margin-bottom: 30px;">
                    Answer these 15 questions to check your understanding of this module. Click on an option to reveal the correct answer instantly.
                </p>
    ...
    for i, q in enumerate(questions):
        html += f"""
                    ...
                    <div class="quiz-q-card">
                        <div class="quiz-q-num">Question {i+1} of 15</div>
...
"""
        for opt_key in sorted(q['options'].keys()):
            ...
            # escape single quotes in text just in case
...
for filename, topic in file_mappings.items():
    ...
    # Get the 15 questions
    if topic == "Monitoring":
        topic_qs = monitoring_questions
    else:
        topic_qs = [q for q in questions_data if q['topic'] == topic][:15]
    ...
    # allowing for different line breaks and spacing.
    # Let's locate the end of `article-content` and the start of `blog-navigation`.
    # Let's use a regex to replace it
    pattern = r'(</div>\s*</article>|</div>\s*<div class="blog-navigation">|</div>\s*\n\s*<div class="blog-navigation">)'
    
    # Actually, let's look at the exact HTML ending in the files.
    # Let's find:
    # </div>
    # <div class="blog-navigation">
    # which is standard.
    # Let's search for "</div>" followed by whitespace and "<div class=\"blog-navigation\">"
    ...
    if match:
        ...
        # We replace this matched string with: quiz_html + matched_str
        # Wait, if we matched `</div> <div class="blog-navigation">`, we replace it with `</div>\n` + quiz_html + `\n <div class="blog-navigation">`
        # Let's make sure it sits outside the article-content container but inside the article container, OR inside article-content itself at the very end.
        # Inside the article container, before blog-navigation is perfect!
        # The matched string starts with `</div>` which closes `<div class="article-content">`. So we want:
        # `</div>` + quiz_html + `<div class="blog-navigation">`
        replacement = f"</div>\n{quiz_html}\n<div class=\"blog-navigation\">"
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print(f"Successfully injected quiz into {filename}!")
    else:
